...
<?php echo CHtml::link(CHtml::image(Yii::app()->request->baseUrl. '/images/exportar.jpg'),'?r=chamado/resSearchporChamadosRealizados&render=resSearchporChamadosRealizados&type=pdf&file=Relatorio_Cha_Por_Realizados'); ?>
<?php echo CHtml::link(CHtml::image(Yii::app()->request->baseUrl. '/images/exportarXLS.jpg'),'?r=chamado/resSearchporChamadosRealizados&type=excel'); ?>
    ...
    if($_GET['Chamado']['cha_termino']) {
        if($_GET['Chamado']['cha_termino'] == 12) {
                ...
                if($_GET['Chamado']['ano_termino']) {
                  ...
                  $cha_terminoQuery = 'AND cha.cha_termino >= "' .$_GET['Chamado']['ano_termino'] . '-0' . $_GET['Chamado']['cha_termino'].'-01" AND cha.cha_termino < "' .$nextYear. '-0' .$nextMonth.'-01"';
              }
        }      
        else{
              if($_GET['Chamado']['ano_termino']) {
                $nextMonth=$_GET['Chamado']['cha_termino']+1;
                  ...
                  $cha_terminoQuery = 'AND cha.cha_termino >= "' .$year.'-0' . $_GET['Chamado']['cha_termino'].'-01" AND cha.cha_termino < "' .$year. '-0' .$nextMonth.'-01"';
              }
        }
    }else if($_GET['Chamado']['ano_termino']) {
        $cha_terminoQuery = 'AND cha.cha_termino >= "'. $_GET['Chamado']['ano_termino'] . '-01-01" AND cha.cha_termino < "' . $_GET['Chamado']['ano_termino'] . '-12-31"';
    }
}
...
WHERE cha.sit_id = 2  AND proc.dep_id=depto.dep_id AND proc.pro_id = cha.pro_id '. $depQuery . ' ' . $cha_terminoQuery . ') T1';
      ...
      $sql = 'SELECT   cha.cha_id AS Chamado,
                ...
                cha.cha_naosolucao
FROM chamado cha, processo proc, departamento depto
WHERE cha.sit_id = 2 AND proc.dep_id=depto.dep_id AND proc.pro_id = cha.pro_id '. $depQuery . ' ' . $cha_terminoQuery;
