...
class ChamadoController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('admin','fechar','resSearchporRealizados'),
				'expression'=>"Yii::app()->user->getState('funcao') == '2'",
			),
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','delete','atualizar','pegarchamado','fechar','index','resSearchporRealizados', 'resSearchporChamadosRealizados', 'pdfresSearchChamado'),
				'expression'=>"Yii::app()->user->getState('funcao') == '1'",
				
			),
			array('allow', // allow admin user to perform 'admin' and 'delete' actions
				'actions'=>array('admin','atualizar','pegarchamado','fechar','index','delete','resSearchporRealizados'),
	...
	public function actionAtualizar($id)
	{
		...
		if(isset($_POST['Chamado']))
		{
			...
			if($model->save()) {
					...
					$msgusu = new YiiMailMessage;
					$msgusu->setBody(
								'<h2>Chamado Atualizado no Sistema de Refrigeracao</h2>'.
								 ...
								 'text/html');
					$msgusu->subject = 'Chamado Atualizado no Sistema de Refrigeracao';
    	...
    	foreach($data as $value=>$name)
    	{
         ...
         public function actionPdfresSearchChamado($render)	{
	       ...
	       //$html = $this->renderPartial($_GET['render'], array('id_contrato'=>$_GET['id_contrato'],), true);
               $html = $this->renderPartial($render, array(), true);                              
	...
	{
                //echo "<script type='teTxt/javascript'>alert('actionResSearchporChamadosRealizados');</script>";
                if(isset($_GET['r'])) {
                    if($_GET['type']=="pdf")
                        $this->actionPdfresSearchChamado ($_GET['r']);
                    if($_GET['type']=="excel")
